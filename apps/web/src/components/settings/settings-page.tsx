...
import { LogOut, User, Shield, Eye, Globe } from "lucide-react";
...
import { SUPPORTED_LANGUAGES, getLanguageNativeName } from "@/lib/languages";
...
function visibilityFormFromSettings(v: VisibilitySettingsResponse | undefined): {
  mode: VisibilityMode;
  workPreferredLocations: string[];
  workSalaryMin: string;
} {
  return {
    mode: visibilityFromSettings(v),
    workPreferredLocations: v?.work_preferred_locations ?? [],
    workSalaryMin: v?.work_preferred_salary_min != null ? String(v.work_preferred_salary_min) : "",
  };
}
...
export default function SettingsPage() {
  ...
  const patchVisibility = useMutation({
    ...
    onMutate: async (body: PatchVisibilityRequest) => {
      ...
      const optimistic: VisibilitySettingsResponse = {
        ...
        preferred_language: body.preferred_language ?? previous?.preferred_language ?? "en",
      };
  ...
  const handleStartEditingVisibility = () => {
    const initial = visibilityFormFromSettings(visibility);
    setMode(initial.mode);
    setWorkPreferredLocations(initial.workPreferredLocations);
    setWorkSalaryMin(initial.workSalaryMin);
  ...
  const handleStartEditingLanguage = () => {
    setSelectedLanguage(visibility?.preferred_language ?? "en");
  ...
  const handleSaveLanguage = () => {
    setLanguageSaveError(null);
    patchVisibility.mutate({ preferred_language: selectedLanguage });
  };
      ...
      <Card>
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>Set your preferred language for the AI assistant and UI.</CardDescription>
              </div>
            </div>
        ...
        <CardContent className="space-y-4">
          {visibilityLoading ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {SUPPORTED_LANGUAGES.map((lang) => (
                    <label
                      key={lang.code}
                      className={cn(
                        "flex items-center gap-2 cursor-pointer rounded-lg border p-3 transition-colors",
                    ...
                    >
                      ...
                      {lang.flag && <span className="ml-auto text-lg">{lang.flag}</span>}
                    </label>
                  ))}
                </div>
              </div>
            ...
            <div className="flex items-center gap-2">
              {SUPPORTED_LANGUAGES.find((l) => l.code === visibility?.preferred_language)?.flag && (
                <span className="text-lg">
                  {SUPPORTED_LANGUAGES.find((l) => l.code === visibility?.preferred_language)?.flag}
                </span>
              )}
              <p className="text-sm text-foreground font-medium">
                {getLanguageNativeName(visibility?.preferred_language ?? "en") || "English"}
              </p>
            </div>
          )}
        </CardContent>
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>Sign out of this device. You can sign back in with the same email.</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent>
