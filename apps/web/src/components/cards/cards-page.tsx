...
function parentFormToPatch(form: {
  ...
  const intentSecondary = form.intent_secondary_str
    ...
    .map((s) => s.trim())
    .filter(Boolean);
  const confidence = form.confidence_score.trim() === "" ? null : Number(form.confidence_score);
  return {
...
export default function CardsPage() {
  ...
  const { patchCardMutation, patchChildMutation, deleteCardMutation, deleteChildMutation } =
    useCardMutations(noop, noop, noop, setEditingSavedCardId, setEditingSavedChildId);
  ...
  const onUpdateParentFromMessyText = useCallback(
    ...
    [editingSavedCardId, editForm, queryClient, setEditForm, language]
  );
  ...
  const onUpdateChildFromMessyText = useCallback(
    ...
    [childEditForm, editingSavedChildId, queryClient, setChildEditForm, language]
  );
  ...
  }, [childEditForm, editingSavedChildId, patchChildMutation]);

  const onDelete = useCallback((id: string) => deleteCardMutation.mutate(id), [deleteCardMutation]);
  const onDeleteChild = useCallback((id: string) => deleteChildMutation.mutate(id), [deleteChildMutation]);

  const isSubmitting = patchCardMutation.isPending || patchChildMutation.isPending;
          ...
          <div className="space-y-4">
            {savedFamilies.map((family, i) => (
              <SavedCardFamily
                ...
                onChildEditFormChange={(updates) => setChildEditForm((prev) => ({ ...prev, ...updates }))}
