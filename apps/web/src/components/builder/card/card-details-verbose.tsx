...
export function CardDetailsVerbose({ card, compact = false, summaryFullWidth = false }: VerboseCardDetailsProps) {
  if (!card) return null;
  ...
  const roles = (Array.isArray(cardAny.roles) ? cardAny.roles : [])
    .map((r) => (typeof r === "object" && r && "label" in r ? String((r as { label?: unknown }).label ?? "") : String(r)))
    .filter(Boolean);

  const actions = (Array.isArray(cardAny.actions) ? cardAny.actions : [])
    .map((a) => (typeof a === "object" && a && "verb" in a ? String((a as { verb?: unknown }).verb ?? "") : String(a)))
    .filter(Boolean);
  ...
  ].filter((r): r is { label: string; value: string } => typeof r === "object" && r !== null && "value" in r && r.value != null && r.value !== "");
      ...
      <div className="mt-3 pt-3 border-t border-border/40 space-y-3">
        ...
        {restRows.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2">
            {restRows.map((r, i) => (
              <DetailRow key={`${r.label}-${i}`} label={r.label} value={r.value} labelClass={labelClass} valueClass={valueClass} />
            ))}
          </div>
        )}
      </div>
