...
- **Overall experience**
  ...
  - Voice is available via **Vapi** (dashboard assistant); after a call, the app commits the transcript through the API so the same card schema is populated as in typed Builder flows.
...
- **Voice flow (Vapi + transcript commit)**
  - Frontend uses the Vapi Web SDK with a **dashboard-configured assistant** (public key + assistant ID).
  - There is **no** custom OpenAI-compatible LLM proxy in this API. When the call ends, the client calls **`POST /builder/transcript/commit`** with `call_id` and/or a full transcript; the server may fetch the transcript from Vapi using **`VAPI_API_KEY`**.
  - The backend runs **`detect_experiences`** + **`run_draft_single`** (and finalization in `services/builder/engine.py`) so voice-generated cards match the typed Builder pipeline.
...
- **Backend (apps/api)**
  - FastAPI-based service with routers for:
    - Builder (`/experience-cards/*`, `/builder/transcript/commit`, etc.).
    - Search (`run_search` and graph view helpers).
  - Experience pipeline orchestrated in `services/experience/pipeline.py`, with supporting modules for clarify logic, embedding, CRUD, and search document construction. Voice-specific orchestration lives in `services/builder/engine.py` (`commit_builder_transcript`).
...
- **Voice agent flow**
  - Conversation and audio are handled by **Vapi** (assistant + model in the dashboard).
  - This API **only** receives the finished transcript (or fetches it by `call_id`) and runs **`commit_builder_transcript`** — see `apps/api/docs/VAPI_CONVAI.md`.
...
- **Voice integration**
  - Vapi handles the realtime session; the API stays simple and stateless for voice (no in-repo custom LLM proxy).
  - Transcript commit reuses the same experience extraction pipeline as the typed Builder.

### 7. Current Gaps / Open Problems

- **UX completeness**
  - Voice and typed flows are separate UX paths (transcript commit vs. interactive clarify); the UI could unify them into a single timeline more explicitly.
...
- **Voice flow**
  - Empty or failed transcript commits (bad `call_id`, missing `VAPI_API_KEY`, or client not sending text) yield no new cards; UX depends on clear client assembly of transcript + `call_id`.
