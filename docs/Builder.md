...
- **User input**
  ...
  - **Real‑time voice** via Vapi (optional; dashboard assistant). After the call, **`POST /builder/transcript/commit`** runs the same **detect → draft** pipeline as typed flows (see `apps/api/docs/VAPI_CONVAI.md`).
...
- **`toggleVoice` callback**:
  ...
  2. Otherwise:
     - Reads **`NEXT_PUBLIC_VAPI_PUBLIC_KEY`** and **`NEXT_PUBLIC_VAPI_ASSISTANT_ID`** (see `lib/vapi-config.ts`).
     - **Lazy imports** `@vapi-ai/web` via `createPatchedVapiClient(publicKey)` — **no** `${API_BASE}/convai` proxy.
     - Registers Vapi event handlers:
       - `call-start` → mark connected; capture **call id** when exposed by the SDK
       - `call-end` → **`POST /builder/transcript/commit`** with `call_id` and/or assembled transcript, then invalidate experience card families
       ...
       - `error` → show user-facing error text
     - Calls `vapi.start(assistantId)` to begin the call.

**Important:** Voice transcript lines appear in the chat UI for continuity; **card creation from voice** happens on **`call-end`** via **`/builder/transcript/commit`**, not on every transcript chunk. Typed Builder flows still drive detect/draft/clarify through the textarea **`sendMessage`** path.
...
- `POST /experience-cards/draft-single`
  ...
  - The **pipeline** handles:
    - Rewrite → extract → validate → persist. **`run_draft_single` intentionally does not embed**; embeddings run when the card is **finalized** or updated via PATCH / fill‑missing flows that call `_reembed_cards_after_update`.
...
### 5.9 Vapi transcript commit

- `POST /builder/transcript/commit`
  - Body: `BuilderTranscriptCommitRequest` (`call_id`, `transcript`, `session_id`, `mode`)
  - Response: `BuilderSessionCommitResponse` (`session_id`, `session_status`, `committed_card_ids`, `committed_card_count`, …)
  - Implemented by `commit_builder_transcript` in `services/builder/engine.py` (detect + draft per experience; finalize per product rules).
...
- **BuilderSessionCommitResponse / BuilderTranscriptCommitRequest**
  - Used by **`POST /builder/transcript/commit`** (voice or batch text commit).
...
### 7.3 LLM Card Models (`Card`, `Family`)
...
Parsed extractor output is turned into `Family` instances via `parse_llm_response_to_families` and related helpers (no separate `ExtractorResponse` wrapper type in code).
...
- After **finalize** and **PATCH / fill‑missing** paths via `_reembed_cards_after_update` (and **`run_draft_single` does not embed** — see §5.4).
...
- Prompt templates in `prompts/experience_card.py`:
  ...
  - `PROMPT_CLARIFY_QUESTION_WRITER` (filled with `canonical_card_json` for the current family)
...
3. **Validates** the plan (`validate_clarify_plan`), which uses `is_parent_good_enough` and per‑question limits.
...
3. **Single experience case**
   ...
   - Pipeline: rewrite → extract families → normalize → persist (**no embed until finalize**).
...
1. **User taps the AiSphere**
   - `toggleVoice` creates a Vapi client with the **public key** and starts the **dashboard assistant**.
2. **Vapi emits transcript `message` events**
   - Builder appends them to `messages` for display.
3. **Concluding / syncing**
   - When `call-end` fires, the client calls **`POST /builder/transcript/commit`** with `call_id` and/or full transcript.
   - The API runs **`commit_builder_transcript`** → detect + `run_draft_single` per experience, then finalization as implemented in the engine.
   - Frontend invalidates `EXPERIENCE_CARD_FAMILIES_QUERY_KEY` so new cards appear in **Your Cards**.

**Voice and text share the same experience card schema and pipeline code**; the difference is **interactive chat** (typed `sendMessage` + clarify) vs **post‑call batch commit** from Vapi.
...
- **Voice behaviour**
  ...
  - Tune Vapi dashboard assistant; adjust **`commit_builder_transcript`** in `services/builder/engine.py` and env vars in `VAPI_CONVAI.md`.
